 ...
 * Handles email sending via MailerSend API.
 *
 * Endpoints:
 *   POST /send    — generic email relay (used by bookings and other services)
 *   POST /support — public support form from dicebastion.com/support
 *
 * Environment variables (Bunny dashboard → script → Env Configuration):
 *   - MAILERSEND_API_KEY      (required)
 *   - SUPPORT_CONTACT_EMAIL   (required for /support — inbox for support form)
 *   - TURNSTILE_SECRET        (required for /support in production)
 *   - MAILERSEND_FROM_EMAIL   (optional, default noreply@dicebastion.com)
 *   - MAILERSEND_FROM_NAME    (optional, default Dice Bastion)
 *   - ALLOW_TEST_BYPASS       (optional, set "true" to accept test-bypass token)
 */

import * as BunnySDK from "@bunny.net/edgescript-sdk";

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const supportRateLimits = new Map<string, [number, number]>();
...
function checkRateLimit(ip: string | null | undefined, limit = 3, windowMinutes = 1): boolean {
  if (!ip) return true;
  const now = Date.now();
...
async function handleSupportContact(request: Request): Promise<Response> {
