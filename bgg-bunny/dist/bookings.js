...
var VALID_WEEKDAYS = /* @__PURE__ */ new Set([
  "Sunday",
  "Monday",
  "Tuesday",
  "Wednesday",
...
function getEffectiveHoursForDate(dateStr, globalStart, globalEnd, rules) {
...
function slotWithinEffectiveHours(startTime, endTime, startHour, endHour) {
...
function isCheckoutPaid(payment) {
  ...
  if (Array.isArray(transactions)) {
    return transactions.some(
      (tx) => tx && typeof tx === "object" && (tx.status === "SUCCESSFUL" || tx.status === "PAID")
    );
  }
  return false;
}
async function callPaymentsWorker(path, init = {}) {
  const paymentsApiUrl = (process.env.PAYMENTS_API_URL || "https://dicebastion-payments.ncalamaro.workers.dev").replace(/\/+$/, "");
...
async function fetchPayment(checkoutId) {
...
async function fetchPaymentByReference(orderRef) {
...
async function getTableTypePrices(tableTypeId) {
...
async function memberHasUsedBenefitInSlot(userEmail, bookingDate, startTime, endTime) {
  const result = await client.execute({
...
function blockEndForCompare(endTime) {
  const normalized = normalizeTime(endTime);
  return normalized === "00:00" ? "24:00" : normalized;
}
function slotOverlapsBlock(slotStart, slotEnd, blockStart, blockEnd) {
...
async function isBookingBlocked(bookingDate, startTime, endTime) {
  const blocksResult = await client.execute({
    sql: `SELECT start_time, end_time FROM booking_blocks WHERE block_date = ?`,
    args: [bookingDate]
  });
  return blocksResult.rows.some(
    (block) => slotOverlapsBlock(
      startTime,
      endTime,
...
async function sendBookingConfirmationEmail(params) {
  ...
  try {
    ...
    const text = template.body_text ? replacePlaceholders(String(template.body_text), templateData) : void 0;
...
async function sendAdminBookingNotificationEmail(params) {
  ...
  try {
    ...
    if (templateResult.rows.length > 0) {
      ...
      text = template.body_text ? replacePlaceholders(String(template.body_text), templateData) : void 0;
      ...
      html = `
        ...
        <table style="border-collapse:collapse;width:100%;max-width:500px;">
          ...
          <tr><td style="padding:6px 12px;font-weight:bold;">Amount</td><td style="padding:6px 12px;">\xA3${amountPaid.toFixed(2)}</td></tr>
...
Amount: \xA3${amountPaid.toFixed(2)}
...
BunnySDK.net.http.serve(async (request) => {
  ...
  try {
    ...
    if (path === "/api/bookings/recent" && request.method === "GET") {
      ...
      const days = Math.min(Math.max(parseInt(url2.searchParams.get("days") || "30", 10) || 30, 1), 90);
      const limit = Math.min(Math.max(parseInt(url2.searchParams.get("limit") || "100", 10) || 100, 1), 200);
    ...
    return jsonResponse({
      table_types: result.rows.map((row) => {
        ...
        try {
          ...
          availableDays = JSON.parse(fixedJson).filter(Boolean);
        } catch (e) {
...
async function getAvailableSlots(date, tableTypeId, userEmail) {
  try {
    ...
    const slotsWithAvailability = await Promise.all(
      timeSlots.map(async (slot) => {
        ...
        if (isMember && userEmail && tablePrices) {
          ...
          slotInfo.price = usesMemberBenefit ? tablePrices.member_price : tablePrices.non_member_price;
        }
        return slotInfo;
      })
    );
  ...
  try {
    ...
    if (is_member_booking && !pricing.usesMemberBenefit) {
      return jsonResponse({
        ...
        message: "You already have a member booking for this session. Additional tables are charged at the standard rate."
      }, 422);
    }
    ...
    const insertResult = await client.execute({
      ...
      args: [
        ...
        amountToCharge > 0 ? "pending" : "confirmed",
        // Track payment status separately
        notes || null,
        now,
        now,
        "confirmed"
        // Status is always confirmed (CHECK constraint only allows confirmed/cancelled)
      ]
    });
    if (amountToCharge === 0) {
      try {
      ...
      if (checkoutData.id) {
        try {
          await client.execute({
            ...
            args: [checkoutData.id, (/* @__PURE__ */ new Date()).toISOString(), order_ref]
          });
        } catch (storeErr) {
...
async function confirmBooking(orderRef) {
  try {
    ...
    let payment = null;
    try {
      payment = checkoutId ? await fetchPayment(checkoutId) : await fetchPaymentByReference(orderRef);
      console.log(
    ...
    if (checkoutId && payment && payment.checkout_reference && String(payment.checkout_reference) !== orderRef) {
    ...
    if (!isCheckoutPaid(payment)) {
      ...
      const txStatuses = Array.isArray(payment.transactions) ? payment.transactions.map((t) => t.status) : [];
      ...
      return jsonResponse({
        ...
        message: hasFailed ? "Payment failed. Please try again." : hasDeclined ? "Your card was declined. Please try a different payment method." : "Payment is still processing."
      });
    }
    const now = (/* @__PURE__ */ new Date()).toISOString();
    ...
    try {
      await sendAdminBookingNotificationEmail({
        ...
        notes: booking.notes ? String(booking.notes) : void 0
      });
    } catch (e) {
      console.error("Error sending admin notification email:", e);
    }
...
async function getRecentBookings(days, limit) {
  try {
    const result = await client.execute({
      sql: `SELECT b.id, b.user_email, b.user_name, b.booking_date, b.start_time, b.end_time,
                   b.status, b.payment_status, b.amount_paid, b.notes, b.created_at,
...
async function updateBookingConfig(request) {
  try {
    const data = await request.json();
    const current = await loadBookingConfig();
    const maxBookings = data.max_bookings != null ? Number(data.max_bookings) : current.maxBookings;
    ...
    const slotDuration = data.slot_duration_hours != null ? Number(data.slot_duration_hours) : current.slotDuration;
    const dayHourRules = data.day_hour_rules != null ? parseDayHourRules(data.day_hour_rules) : current.dayHourRules;
    if (!Number.isInteger(maxBookings) || maxBookings < 1 || !Number.isInteger(startHour) || startHour < 0 || startHour > 23 || !Number.isInteger(endHour) || endHour < 1 || endHour > 24 || !Number.isInteger(slotDuration) || slotDuration < 1 || slotDuration > 12 || startHour + slotDuration > endHour) {
      return jsonResponse(
        {
          error: "invalid_config",
          message: "Check max_bookings, start_hour, end_hour, and slot_duration_hours (start + duration must fit within end)."
        },
        400
      );
    }
...
async function createTimeBlock(request) {
  try {
    const data = await request.json();
