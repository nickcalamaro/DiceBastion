...
/** Add --figure-bg-image to existing figures so sync picks up backdrop without re-editing posts. */
function applyFigureBackdropVars(html: string): string {
  if (!html || !html.includes("blog-inline-figure")) return html;
  return html.replace(
    /<figure\b([^>]*class="[^"]*blog-inline-figure[^"]*"[^>]*)>([\s\S]*?)<\/figure>/gi,
    (block, attrs, inner) => {
      if (attrs.includes("--figure-bg-image")) return block;
      const src = extractImgSrcFromHtml(inner);
      if (!src) return block;
      if (/\bstyle=/i.test(attrs)) {
        const merged = attrs.replace(
          /\bstyle=(["'])([\s\S]*?)\1/i,
          (_m, quote: string, styles: string) =>
            ` style=${quote}${styles}; --figure-bg-image: url('${src.replace(/\\/g, "\\\\").replace(/'/g, "\\'")}')${quote}`
        );
